...
& $Python (Join-Path $ProjectRoot 'tools\visualization\build_video_index.py') --report-root $ReportRoot --comparisons
if ($LASTEXITCODE -ne 0) { throw 'Comparison encoding or validation failed.' }
Write-Output "Comparison videos complete: $(Join-Path $ReportRoot 'videos\comparisons')"
